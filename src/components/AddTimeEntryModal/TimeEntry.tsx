...
const AddTimeEntryModal: React.FC<AddTimeEntryModalProps> = ({
  ...
  const [formData, setFormData] = useState({
    project:
      (initialData?.project as any)?._id ||
      (initialData?.project as string) ||
      (initialProject?._id || (initialProject as any)?.id) ||
      "",
    date: initialData?.date
  ...
  const modal = (
    ...
    >
      <div
        ...
        aria-hidden="true"
      />
      <div className="relative z-[10000] bg-slate-900 border border-slate-800 rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="bg-slate-950 border-b border-slate-800 p-6">
          ...
          <p className="text-slate-400 mt-1">Track your work hours and tasks</p>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div>
            ...
            <select
              ...
              onChange={handleChange}
              disabled={isProjectDisabled}
              className={`w-full px-4 py-3 rounded-lg bg-slate-950 border border-slate-700 text-slate-200 focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all ${isProjectDisabled ? "opacity-60 cursor-not-allowed" : ""}`}
              required
            >
              <option value="">Select a project</option>
              {projects.map((project) => (
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                value={formData.date}
                onChange={handleChange}
                className="w-full px-4 py-3 rounded-lg bg-slate-950 border border-slate-700 text-slate-200 focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all"
                required
              />
            </div>
            <div>
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-slate-800">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2.5 border border-slate-700 rounded-lg text-slate-300 hover:bg-slate-800 hover:text-white transition-colors"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="px-5 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-500 transition-all shadow-[0_0_15px_rgba(37,99,235,0.3)] border border-blue-500/50"
            >
              Save Entry
            </button>
          </div>
